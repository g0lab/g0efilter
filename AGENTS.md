...
- `dashboard/ui/` is the dashboard frontend (Vite + Svelte 5 + TypeScript, pnpm).
  ...
  (via `dashboard/ui/embed.go`, `go:embed all:dist`); after changing `ui/src/`, run
  `pnpm build` there and commit the regenerated `dist/`. The Docker image rebuilds the
  UI itself, so a stale commit can't ship.
