...
func localHelmChart(t *testing.T, chart string) string {
	...
	once, ok := prepare.(func() (string, error))
	if !ok {
		t.Fatalf("chart cache holds %T for %s", prepare, chart)
	}
...
func prepareLocalChart(chart string) (string, error) {
	...
	temporaryChart, err := copyChartWithoutDependencies(chart)
	if err != nil {
		return "", err
	}
	...
	//nolint:gosec // temporaryChart comes from os.MkdirTemp, not from test input
...
// copyChartWithoutDependencies copies a chart to scratch space with its packaged
// dependencies and lock removed, ready to be repointed at a local library chart.
func copyChartWithoutDependencies(chart string) (string, error) {
