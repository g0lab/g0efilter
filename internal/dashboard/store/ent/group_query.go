...
func (_q *GroupQuery) QueryInstances() *InstanceQuery {
...
func (_q *GroupQuery) WithInstances(opts ...func(*InstanceQuery)) *GroupQuery {
	query := (&InstanceClient{config: _q.config}).Query()
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withInstances; query != nil {
		if err := _q.loadInstances(ctx, query, nodes,
			func(n *Group) { n.Edges.Instances = []*Instance{} },
			func(n *Group, e *Instance) { n.Edges.Instances = append(n.Edges.Instances, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadInstances(ctx context.Context, query *InstanceQuery, nodes []*Group, init func(*Group), assign func(*Group, *Instance)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(instance.FieldGroupID)
	}
	query.Where(predicate.Instance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.InstancesColumn), fks...))
	}))
